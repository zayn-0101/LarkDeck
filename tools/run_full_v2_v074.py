# SNAPSHOT（v0.7.4）：用于证据 5b95374 的 scratch runner（路径已脱敏）。

#!/usr/bin/env python3
"""v0.7.4 P3 全量变异（v2：先验片、后盖章；每轮独立目录 + 证据哈希 + 禁旧 at 白名单）。
...
只读校验 + 只写 `~/.larkdeck-scratch/v0.7.4/`；本脚本不 commit、不 checkout、不 sleep 轮询
...
SCRATCH = pathlib.Path.home() / ".larkdeck-scratch" / "v0.7.4"
SHARDS = 6
...
def main() -> int:
    ...
    env_fp = {
        ...
        "release_script_sha256": (sha256_file(pathlib.Path.home() / ".larkdeck-scratch" / "release-v0.7.4.py")
                                  if (pathlib.Path.home() / ".larkdeck-scratch" / "release-v0.7.4.py").exists() else ""),
