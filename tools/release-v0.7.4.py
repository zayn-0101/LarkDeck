# SNAPSHOT（v0.7.4）：用于证据 5b95374 的 scratch release（路径已脱敏）。

#!/usr/bin/env python3
"""v0.7.4 发布日执行器（用户终验通过后才跑；默认 dry-run，`--go` 才动手）。
...
用法::
    ...
    $PY ~/.larkdeck-scratch/release-v0.7.4.py            # dry-run：只做只读核对
    $PY ~/.larkdeck-scratch/release-v0.7.4.py --check    # 同上（显式）
    $PY ~/.larkdeck-scratch/release-v0.7.4.py --go       # 真跑：打 tag/push/release/部署/重启
...
TAG = "v0.7.4"
TAG_MSG = ("LarkDeck v0.7.4：Hermes 本地化系统/命令回复（/reset、/new、/reload-*、/stop、/reasoning 等）"
           "与 /larkdeck 自诊卡命中登记 header 后整卡静默，真实回合 ✅ 一个字不动；"
           "长任务/多卡终局不再渲染空白面板 shell；V074 系列变异全红（数量见账本）。")
...
def main() -> int:
    ...
    notes = REPO / "docs" / "releases" / "v0.7.4.md"
    if not notes.exists():
    ...
    if "## [0.7.3] - " not in changelog or "## [Unreleased] - v0.7.4" not in changelog:
        raise SystemExit("❌ CHANGELOG.md 缺「[0.7.3] 已收口 + [Unreleased] - v0.7.4 已开」"
    ...
    drift = subprocess.run(
        ["git", "diff", "--name-only", "--diff-filter=MD", f"{fa}..HEAD", "--",
         "core", "tests", "plugin.yaml", "tools", "docs/audits/v0.7.4",
    ...
    d = pathlib.Path.home() / ".larkdeck-scratch" / "v0.7.4"
    ...
    runner_now = hashlib.sha256((pathlib.Path.home() / ".larkdeck-scratch" /
                                   "v0.7.4-run_full_v2.py").read_bytes()).hexdigest()
    ...
    if captured:
        ...
        md = REPO / "docs" / "audits" / "v0.7.4" / f"p3-full-run-{fa}.md"
    ...
    _, out = run(["gh", "release", "create", TAG, "--title", f"LarkDeck {TAG}",
                  "--notes-file", "docs/releases/v0.7.4.md"], timeout=300)
