...
| 措施 | 说明 |
| --- | --- |
| **6 分片 + 完整模式（照常 first-red）** | ✅ **本批实测（2026-09-22，`adea7cb`）：6 片 / 499 条变异 + 12 条对照 ⇒ 墙钟 ≈25 分钟**（20:40→21:04），**坏 0**（无 💥/🟢/❓），合并 `✅ 499 条通过`、`at` 全 = `adea7cb`、`full_audit_tree=567f47ec`。⚠️ 计划原估 9–11 min 只在**机器空闲**时成立；本机同时有桌面/harness 负载 ⇒ 实测 25 min。⚠️ 另有**一次无效运行**（起跑后我又提交了文档 ⇒ 分片 `at` 与 HEAD 不符）被审计 B2 判无效并停掉，如实记录、不作为证据。⚠️ 门禁硬超时 **45s→90s**（并行负载下 `test_units` 47–53s ⇒ 曾产生 10 条 `💥` 假坏） |
...
### P4/P5（断言·变异·夹具·验证机制）三路对抗审计 —— 全部已回、逐条处置

| 路 | agent id | 发现（编号+严重度） | 处置 |
| --- | --- | --- | --- |
| A2 局部回退取证 | `dc5a9f5e` | 对 7 条语义声明逐条做局部回退：A1 两处 seed / 中间帧省略 / 嵌套轮 / 回合结束与 `/stop` 定稿 / 幂等顺序 / B1 / C1 / D1′ ⇒ **全部「删掉即红且红在断言上」**；唯一洞：性能缓存 `_LD_CK_REQUESTS_CACHE` 的移除**无门禁**（低；等价优化） | 7 条全部确认成立；缓存记「低、无门禁」并登记（正缓存理论上可掩盖运行期 SDK 消失，生产里 SDK 静态 ⇒ 低） |
| B2 验证机制 | `d7b03b17` | ①**高**：起跑后提交文档 ⇒ 分片 `at` 与 HEAD 不符，整轮作废；②**高**：断点续跑坏（陈旧 seed 让 `--delta` 跳过，而日志被 `wb` 截断 ⇒ 名字两处无证据）；③中：无起跑时脏树/HEAD 固定、launch 不清陈旧账本/无锁、日志块缓冲（18 分钟仍 0 字节）、merge 单独不强制「6 片全新鲜」；④低：`merge.sh` 漏 `--allow-at`（漂移） | ①②③④**全部落地**：`launch_full_v2.py` 起跑前清陈旧账本/日志 + 校验工作树干净 + 记 `launched_head.txt` + `-u` 不缓冲；`merge.sh` 补 `--allow-at` + 分片数动态；超时 45→90s；那轮无效运行已停并**如实记录**。⚠️「合并器不强制 6 片」由发布脚本 1a（`at==stamp` 且 `n_inh==0`）兜住 |
| C2 文档/诚实性 | `f92e89a8` | 10 条：资产落库后**反向 stale**（v2 文档/README/plugin.yaml 还写「待选定/过渡态」）、§7-C 的「页脚旧描述四处」未全兑现、README 仍把 `panel_expanded` 当 no-op、`patch` 键在 structured 下无效未写全、§8 探针覆盖不足、发布脚本 `--check` 不覆盖 `.deploy==HEAD` 校验、数字 stale、4 处代码注释旧口径 | 文档侧 **13 处已修**（提交 `403964f`/`36c3929`/`403964f` 序列）；探针默认改**预览**（`--send` 才发，防误发）+ 补 §8-5 对照行；发布脚本 1c 移进 `--check` 覆盖范围；**4 处代码注释**（`context.py`/`i18n.py`/`adapter.py`/`mutate_check.py`）留到发布后收尾（现在改会动指纹、作废刚跑完的账本） |
