...
* **P6「全量门禁」的范围与预算**（审计 A：全矩阵在负载机器上约 6.5h，必须写清口径）：
  ...
  helper/fixture）+ 周期性全量直跑背景刷新账本（`--ledger-status`：484/484、待跑 0；**
  2026-09-22 实测**：全量直跑分 4 片并行 ≈14 分钟（两轮：下午那轮另有 8 条负载 💥 定向复跑 ≈3 分钟））；
...
| 图标「更全面」的**剩下三批**（用户 2026-09-22：「CLS 只在一部分场景用了这些图标，我们应该更全面」）：① 面板标题的两个区段（💭/🛠️）改线性图标 —— 一条 `plain_text` 放不了两个前缀图标，要么拆两列 `column_set`，要么保留 emoji（现为 CLS 同款）；② 页脚四段（状态/耗时/模型/ctx）各带图标 —— 需四列分栏（元素 +4，窄屏可能折行）；③ 澄清卡（问题/已答复/过期/重试）、`/larkdeck status|config` 分区标题、降级提示 | 本仓 | 真机看过 P3.1 效果后再定批次；每个新位置都要「断言 + 变异 + 重验」 | v0.7.3 |
