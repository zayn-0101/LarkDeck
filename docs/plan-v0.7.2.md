...
1. **页脚不再有 🔖 短码** —— 用户从未要求过它（是 V2/V3 阶段内部审计 C1 加进来的）。
   ...
   > ⚠️ **本行已被 2026-09-22 批次（B1）取代**：段前缀 `⏱`/`🤖` 已去掉变纯文本；
   > 现行口径见 `docs/plan-v0.7.2-panel-ux.md` §0（B1）。本文件保留作历史证据。
...
2. **加载指示对齐 aiduPOP `_loading_element`**：`div` + `custom_icon(img_key)` + `text: " "`
   ...
   **硬编码同一个 key**，三家都没有 spinner 上传代码（它们的 `upload_image` 只服务正文远程图）。
   ...
   > ⚠️ **「不做上传」已被 D1′ 推翻（用户 2026-09-22）**：工具行「运行中」也要动图，用户要求
   > **像 aiduPOP 那张** ⇒ 我们自制一张三点动图、用自己的 app 上传一次。
   > 现状与进度见 `docs/audits/v0.7.2/loading-asset-v2.md`。本行保留作历史证据。
...
| **P3 图标** | 主表 28 条逐条等于 CLS（含顺序）+ `terminal` 登记偏差；`tests/check_cls_alignment.py` 直连 CLS 源码；三臂布局探针（**同一文本、只变 icon 放法**）→ 用户选「乙 = `markdown` 前缀图标」；线性 `_outlined` + 统一灰，并扩展到详情行/错误块/折叠提示 | 对照表逐条 ✅ + CLS 对齐门禁 OK；变异 `V4-46/56/57/58/59/60/61` 实红；落地探针卡 `om_x100b64159f75b0a0c2f35ecdf3f0d36` 发送成功 | 代码 ✅ / 用户目视「基本都可以」✅ |
| **P7 面板 UX 定版**（2026-09-22 新增，计划外） | 用户拍板 A1/B1/C1/D1′/E1：运行中展开+收尾折叠、嵌套轮当前轮展开、中间帧不带 `expanded`、页脚去 emoji、运行中蓝色、自制 GIF（工具行 + 加载指示同一张，自己上传）、保持正文在上 | 见 `docs/plan-v0.7.2-panel-ux.md`（含逐字文案、省时方案、回退开关、审计记录） | **计划阶段** |
...
| 页脚「每段一个官方图标」（用户 2026-09-22 点名要试，代号 B4）：`markdown`/`div` 每段只有一个前缀图标槽 ⇒ 只能 `column_set` 多列；早先探针实测横向 179px 空隙，窄屏可能折行 | 本仓 | 先发探针卡调列宽/对齐，真机目视过关再实现 | v0.7.3 |
| 「自绘动画」代号 D5：不用任何资产，用 Unicode 字形（`◐◓◑◒` / `⠋⠙⠹`）+ enum 色（随主题）由插件每 ~400ms 重写卡片驱动。⚠️ 会额外占用写卡配额（≈2 次/秒；我们现状最坏已 ≈8 次/秒，卡级上限 10），且需先真机确认字形渲染 | 本仓 | 先做一次真机写卡预算实验（量滑窗命中率）+ 字形探针；实现要走「发一次、失败即丢、绝不重试」的通道 | v0.7.3 |
| 「过程在上、正文在下、按时间交错」（CLS 结构，代号 E2）：需要把正文拆成多段 segment 才能与工具块交错，属结构调整 | 本仓 | 结构与渲染重构评估 + 用户真机看 CLS 截图确认要不要 | v0.7.3 |
| 嵌套 `element_id` 能否被**单独** `partial_update`（决定"用户手动展开一个已结束的推理轮后，能不能不被下一帧按策略收回"）—— 现状只能整块重写 children | 本仓 | 一次真机探针（对嵌套 id 发 partial_update，看是否生效/报错）+ 若不支持则如实写进文档 | v0.7.3 |
| 加载指示与工具行「运行中」动图目前仍是**图片资产**（颜色写死、不随主题）；若要彻底主题化只能走 D5 | 本仓 | 同上（D5） | v0.7.3 |
