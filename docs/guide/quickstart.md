...
- 如果没有，先按 [安装指南](installation.md) 的环境要求与“准备机器人凭据”完成。
...
1. 私聊机器人发 `/larkdeck status`。预期收到一张七行结论卡，首行类似 `🃏 larkdeck v<当前版本> · 传输 cardkit`，表格里能看到 `平台接管 ✅ 已接管` 与 `钩子 ✅ 8/8 已挂`；需要更细的诊断时发 `/larkdeck status --detail`。
