...
本文面向日常使用 LarkDeck 的用户：说明与卡片相关的命令、大致输出，以及哪些命令只在网关空闲时才能执行。
...
下面是一张七行结论表：平台接管、钩子、推理显示、最近写卡、写卡失败 / 掉回纯文本、错误码、已运行。每格都是「标记 + 标签：值」：`✅` 表示有证据或计数为零，`⚠️` 表示明确异常，`○` 表示没有记录，`🕒` 是运行时长。

日常看这一张就够。遇到异常要查原因时，加 `--detail`：

```text
/larkdeck status --detail
```

它会展开三段完整诊断：

- 能力与链路：能力探测、钩子、命令注册、模块世代，有明确异常才带 `⚠️`。
