...
### 第十路审计（45 条变异）：无阻断、3 条【中】，全部已处理

三条【中】的用户可见症状**是同一个**（`/stop` 之后卡片不变色），都属于「第九路想修、只修了一半」：

| 级别 | 问题 | 修法 |
|---|---|---|
| 中（**我自己引入的回归**） | 近似阈值 `_MAX_TRACKED_TEXT` 被当**充分条件**用，而真实卡片开销随正文形状变化（`config.summary` 会 flatten + 截断 120 字）⇒ 反斜杠 / `\t` 形状的 ~127KB 正文被判「存不下」丢掉，**而那张卡其实发得出去也画得上色**（审计反事实实测：载荷 127975B ≤ 128000 且有色）。窗口 50~284 字节 | 判据换成 :func:`_stop_redraw_would_paint`：**直接构造那张带状态小面板的卡**，要求「≤ 飞书实测硬上限」且「卡里真有那个面板」。近似阈值降级为「明显没戏」的快速排除（`_HOPELESS_BYTES`） |
| 中 | 反方向同样的缝：纯 CJK 在 JSON **126890** 字节就已经画不上色，比阈值 126976 **低 86 字节** ⇒ 那 86 字节的带里「留下了正文、却发出一次没有颜色的 patch」，而且**没有任何日志提到颜色** | 同一个真判据把这个带也堵上；并且 `_ld_redraw_one_stopped` 在载荷真没带上颜色时打一条限流 WARNING（`_log_no_colour_once`），不再打那句误导的「已把中止态重绘到卡片」 |
| 中 | `mutate_check` 的崩溃分类仍把**纯语法错误**算成判别力证据（`check_override`/`check_hooks` 捕获加载器异常后只打自己的友好文案，`FAIL: ` 与真断言失败无法区分） | 分类改成：green 必须看到该门禁的收尾语；red-assert 要求看到它的失败标记；命中「加载失败 / 语法错误 / import 错」标记 ⇒ **red-crash（不算证据）**。实测三态：语法错误→四门禁全 red-crash、真断言失败→red-assert、干净基线→全 green |
| 低 ×8 | 判据口径去掉 `ensure_ascii=False` 全绿（真去掉会把能上卡的 CJK 上限从 42324 字砍到 21162 字）· `_CARD_BYTES_OVERHEAD` 加倍全绿（把上面那条窗口拉宽到 930~1308 字节）· 降载日志**调用点**传 `(0,0)` 或档位写死 `"ok"` 全绿 · 瞬态码表删掉官方限流码 / 混入确定性拒收码全绿 · 防漂移闸门假红又假绿 · 测试文件里一处**孤儿 docstring**（改名时吃掉了 `def` 行）· 探针用原始字节比对 JSON 口径阈值 · 探针把本地构造的 `in_final` 说得像对乙卡的断言 | 逐条补门禁：口径钉成数字（`"汉"` = 5 字节）· 调用点日志断言（档位/元素/字节都来自实参）· 瞬态码表白名单断言 · 防漂移闸门改成**全包只允许一处数字定义**（既不看写法也不漏 `cards.py` 内部的复制）· 补回 `def` 行 · 探针两个口径都打 · 探针那句改成如实的措辞 |
...
0. **断言被「更早的另一个失败」顺带满足（本轮新增用例当场中招）。**
   给「建实体回 `code=0` 但 `card_id` 是空串 ⇒ 必须 fail-open」写用例时，我把它放在
   ⑤（「拿不到 SDK ⇒ fail-open」）**后面**，而 ⑤ 把请求构造换成了「没有 SDK」的哑对象 ——
   那一帧本来就因为没 SDK 返回了 `False`，两条断言全被满足，闸门根本没执行到，
   变异 `CK11` 因此全绿。移到 ⑥ 之后（哑对象已恢复）并补一条
   `calls["create"] == 1`（**被测的那一步真的被调用到了**）才抓住。
   与下面第 1 条合起来是同一个教训的两个形态：**门禁绿了，但绿的原因不是你要证的事**
   —— 变异检查器答不了「它为什么绿」，这一层只能靠人想清楚依赖顺序（`docs/lessons.md` 推论 19）。
...
### 第十二路审计（审「翻默认 + 面板结构 + 自检自报」这批）：1 条中 + 3 条门禁缺口

审计对象是**上一个commit之后新出现**的那部分（第十一路审计没覆盖到）：
默认翻成 cardkit、`cardkit_entity_card(panel=…)`、`ck_panel` 跟随写入、字节预算闸门、
两条限流告警、启动自检里新加的自报。结论：**无阻断**，一条【中】已修。

| 级别 | 问题 | 为什么是缺陷（用户可见后果） | 修法 |
|---|---|---|---|
| 中 | **CardKit 的写接口完全没有限流退避** —— 而 patch 路径的 `_ld_update_card` 早就有（`_TRANSIENT_BACKOFF`，0.1/0.3/0.6）。`_ld_ck_write` / `_ld_ck_create` / 发实体卡三处都是裸 `_run_blocking` | 撞上一次限流（`230020`）这一帧就返回 `False`，内核随即**停用本回合的 native**：用户后半段看到的**不再是逐字卡片**（可能变成若干条普通消息）。同一个错误码在两条传输上后果完全不同 —— 纯粹的不对称 | 新增 `_WRITE_RETRY_CODES`（只含频率限制类 `230020`/`99991400`）+ `_ld_write_with_retry()`，用在元素写入与建实体/发实体卡三处 |
| 中 | **退避表里那两个 CardKit 专属码不能重试**：`300309`（流式会话已关闭）与 `300317`（序号不匹配）是**结构性状态**，原样重发**不可能成功** | 若把它们混进「可重试」，每次都会白等 1 秒（0.1+0.3+0.6）才回落 —— 延迟了回落、还让日志看起来像限流问题 | 分成两张表：`_TRANSIENT_CODES`（沿用）与 `_WRITE_RETRY_CODES`（**可安全重试**的幂等码）。测试 ⑧b 直接断言 `300309` **只写一次** |

**这一条特别值得记**：`_TRANSIENT_CODES` 的注释里，2026-09-13 就写着
「`300309`/`300317` 放在这里是为**将来可能的 CardKit 传输**预先收口」——
而 CardKit 在同一个文件里成为**默认传输**时，那句「收口」**仍然是空的**：
元素写入路径一次都不重试（连 `230020` 都不）。**承诺写在注释里、而实现跟上了名字却没跟上行为**，
这类「注释里的将来时」是审计的富矿。

**第二路同批审计**（独立跑的，专门审「门禁的判别力」）另外挖出 3 条缺口，
连同它实测到的 2 条**行为缺陷**一并处理（附录见下）：

| 级别 | 问题 | 用户可见后果 | 修法 |
|---|---|---|---|
| **高**（真机已复验修好） | **cardkit 把回复锚点整个丢掉**：`_ld_ck_create` 的签名里没有 `reply_to`，发实体卡走的是顶层 `message.create`。而核心**每次**都带 `reply_to`（生产上恒有值），patch 传输那边交给官方 `_send_raw_message` ⇒ **引用回复** | 翻默认之后**每一次回答都不再挂在提问下面**（引用框消失）；话题群里更糟 —— 官方代码宁可放弃回落也不发顶层消息（`skipping top-level fallback to avoid creating a new topic`），我们却是无条件顶层发送 | 有锚点走 `im.v1.message.reply`（`reply_in_thread=False`，与 patch 路径同形），无锚点才顶层发送。**真机判据不是 `code=0`，而是读回消息的 `parent_id`**：实测 `message.reply=1 · message.create=0`，读回 `parent_id == 锚点 id` ⇒ 真的是回复 |
| **高** | **`streaming_mode` 零门禁**：把实体卡的 `streaming_mode` 改成 `False`／删掉这个键，**四门禁全绿** | 它就是这次翻默认的**唯一理由**（逐字打字机的总开关）。改坏了本地全绿、真机上「卡片再也不逐字了，而且没人知道为什么」 | 断言建实体 JSON 的 `config.streaming_mode is True`（`calls["entity"][0]` 本来就存下来了）+ 变异 `CK20` |
| 中 | **发实体卡没有 `uuid`**（官方发送路径一直填）：`uuid` 是请求去重键，而我们这条路径**会退避重试** | 「响应丢了但其实发成功了」⇒ 重发一次 ⇒ DM 里多一张实体卡，而那张卡永远收不到元素写入（永久停在「⏳ 正在生成…」） | `send_entity` / `reply_entity` 都带确定性 `uuid = ld-msg-<card_id>`；断言改成**直接验真 SDK 请求构造**（⑬ 段）|
| 中 | 建实体那道硬上限闸门守的是**空正文的 seed 帧**（核心传 `""`），而真正会长大的是后面每帧的累积全文 —— 那里**没有任何判断** | 15 万字节的长回答：建实体顺利通过，元素写到某个大小才被拒 ⇒ 到点掉链 | 写正文元素前加同源闸门（超了立刻 fail-open + 限流告警），别白花一次往返 |
| 低 | 探测到的**门禁自比较**：⑫ 段断言的是**替身**给出的 uuid（替身自己填的），把生产代码里的 `.uuid(...)` 删掉照样绿 —— `CK21` 曾因此全绿 | 断言看着在守，实际守的是我写的桩 | ⑬ 段直接调**真** `_ld_ck_requests()` 验请求对象（SDK 缺了就打印跳过原因，不让「没跑」被读成「通过」）|

**这轮最值钱的一条方法论**（已写进 `docs/lessons.md` 推论 19 的第 3 形态）：
**断言可能观测的是我自己写的替身**。替身里 `send_entity` 填了 uuid，于是「生产代码填没填 uuid」
这件事**根本没有被测到** —— 变异 `CK21` 全绿就是证据。修法是让断言直接打在**真 SDK 请求对象**上
（`_ld_ck_requests()` 的产物），而不是打在替身的返回值上。

三条门禁缺口（都是「撤掉修复四门禁全绿」）已全部补上变异并逐条验红：

| 变异 | 撤掉的是什么 | 谁抓 |
|---|---|---|
| `CK12-元素写入不再退避重试` | 元素写入的退避（命中限流即整回合掉 native） | `test_units.py` ⑧a/⑧c |
| `CK13-限流退避表里混进结构性码` | 「结构性码不重试」这条判断 | `test_units.py` ⑧b（断言只写一次） |
| `CK14-建实体不再退避重试` | 建实体的退避（限流一次就没卡了） | `test_units.py` ⑧d |

---

## 2026-09-13 真机实验：**「结构性写入必关流式会话」这条结论太窄了**（重大更正）

**起因**：调研同类插件时发现 `aiduPOP` 与 `lark-hls-v2` 都在**流式会话进行中**调
`cardkit.card.batch_update`（`add_elements` / `insert_before` / `partial_update_element` /
`delete_elements`），并且之后继续用 `card_element.content` 打字；它们还把 `300309` 登记成
**瞬态可重试**码。这与我们写在 `AGENTS.md`/README 里的「任何结构性写入都会关闭流式会话」
直接冲突 —— 两说不能同时成立，只能上真机。

**实验**（`/tmp/probe_ck_stream_ops.py`，走真 SDK + 真卡片，建实体后每步都紧跟一次
`card_element.content` 写，**以「后一次写入的返回码」作判据**）：

| 流式期间的操作 | 返回码 | 之后写 `content` |
|---|---|---|
| （基线）`card_element.content` | `0` | ✅ `0` |
| `card_element.patch`（`partial_element` 局部更新） | `0` | ✅ `0` |
| `card.batch_update`（`partial_update_element`） | `0` | ✅ `0` |
| `card_element.create`（**往卡里新增元素**） | `0` | ✅ `0` |
| `card_element.update`（整元素替换） | `0` | ✅ `0` |
| `card.settings`（`streaming_mode: false`） | `0` | ❌ **`300309 streaming mode is closed`** |

**正确结论**：关掉流式会话的是**整卡替换**（`message.patch` / `card.update`），
而 **CardKit 自己的元素级 / 批量接口在流式期间可用、且不关闭会话**。
原来那句「任何结构性写入」是把两类接口混成了一句 —— 而这句错话**曾经支撑了一个设计决定**
（「结构必须在 `card.create` 时定死」）。**设计决定本身仍然成立**（我们的实现只写两个元素、
收尾用 patch），但**依据要改**：不是「不能改结构」，而是「当时没验过还能怎么改」。

第二个实验（`/tmp/probe_ck_live_edit.py`）把这条能力**做成了用户看得见的卡**：
流式期间写 3 帧正文 → `card_element.create` 插入一个新元素 → `card_element.patch` 把折叠面板
边框改成 `yellow` → `card_element.patch` 更新面板正文 → 继续写正文（全部 `code=0`）→
最后只用 `settings(streaming_mode=false)` 收尾。全部 `code=0`，且**每一改之后正文都还写得进去**。

**这条更正解锁的能力**（此前被我们自己判成「做不到」）：

| 能力 | 原来为什么不做 | 现在 |
|---|---|---|
| 流式期间**加元素**（页脚、状态行、工具步骤） | 以为会关会话 | ✅ 可做（`card_element.create`）|
| 流式期间**更新面板内容**（不整卡 patch） | 以为只能等收尾 | ✅ 可做（`card_element.patch`）|
| 流式期间**上状态色**（面板 `border.color`） | 只敢在收尾帧上 | ✅ 可做（`card_element.patch` 只传 `border`）|
| 面板拆成**多个元素**（每轮/每工具一个） | 结构定死 | ✅ 可做（`create` + `insert_after`）|
| 会话列表**预览文案**随进展更新 | 只会收尾 patch | ✅ 可做（`card.settings` 只写 `summary`，不关会话）|

✅ **2026-09-13 已验收（用户肉眼，截图为证）**：「接口收下了」确实等于「客户端画出来了」——
那张实验卡上，**流式期间 `create` 插入的新元素**、**`patch` 改成黄色的面板边框**、
**`patch` 更新的面板正文**三者全部正确渲染，面板展开/收起也正常。
⇒「流式期间加元素 / 改面板 / 上状态色」这条路**成立**，可以做「阶段 10」。
（可复现的探针见 `tests/probe_ck_stream_ops.py`。）
