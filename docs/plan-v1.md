...
| R3 起 | 未开始（R3 的「面板内多元素」仍在等附录 C 第 1 眼的结论） | |
...
| `DEAD` | **这一批里**的装饰 `element_id` 进 `ck_dead`（`batch_update` 的返回码是**卡级**的，看不出哪个 action 坏 ⇒ 只能整批标死），跳过它们继续 + 限流 WARNING | `True` | 卡继续逐字，装饰冻结到本回合结束；**收尾帧的整卡替换会补齐**（面板/页脚/状态色） |
