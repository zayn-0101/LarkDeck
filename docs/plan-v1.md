...
| R2 流式页脚 + 状态色 | ✅ 完成（页脚元素**建实体时就建好**；装饰**在流式期间**逐帧更新、**内容没变就不写**（稳态每帧 1 次写，每帧上限 2 次 = 1 batch + 1 正文，正文最后写）；装饰失败 `DEAD` + WARNING 而帧仍 `True`，只有正文失败才 fail-open。状态色载体（面板边框）也是建实体时就进卡（灰边起步），收尾那一帧才上色 —— **流式中途换色没做**：`batch_update` 的 partial update 不允许改结构性字段（`border.color`），要改得上 `card_element.patch`，而流式期间的状态值只有「运行中」一种，做了没有收益（列入 R3/R5 之后） | 变异 **120/120 全红 + 5 对照绿** · 门禁 131/131 + OVERRIDE/HOOKS/CLARIFY E2E 全绿 · 真机 `--cardkit-prod`：4 帧 = 4 次正文写入 + **2 次 batch**（首帧 `panel_body+footer`、面板变化那帧**只有 `panel_body`**，页脚没变就不写）、**全 `code=0`**、序号严格递增 · `tests/golden_cardkit_trace.json` 的 diff 就是本次行为变更的声明 |
...
断言三条：① 数字**写死**（10 与 2，这是「照官方口径留余量」的显式决定）；
