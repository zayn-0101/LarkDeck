...
| P9 整卡 patch 能否覆盖 **CardKit 实体卡**的消息 | **能**：`code=0`，且随后写元素得 `300309`（会话确实被替换掉）⇒ `DEGRADE` 车道可做（探针 `--lanes`） |
...
| R5 健壮性 | ✅ 完成（撤回守卫 `_ld_drop_message`；`DEGRADE` 车道：卡级死法 `{300309,300313,300317}` ⇒ 清 `card_id` + 整卡 patch 续写**同一张卡**；元素闸门确认已在 R2 落地；缓存上界 `_MAX_TRACKED=512`（按最近活动淘汰）+ 新增上界用例；批量失败按 msg 点名的坏 id **只标死那一个**） | 变异 **131/131 全红 + 5 对照绿** · 门禁 133/133 + OVERRIDE/HOOKS/CLARIFY E2E 全绿 · 真机两条：`probe_ck_stream_ops.py --lanes`（`batch` 混坏 id ⇒ `300313` 且 msg 点名；整卡 patch 能覆盖实体卡消息 ⇒ 会话被替换）与 `probe_render.py --degrade-lane`（**生产路径**：手工关会话后那一帧 `True` + `ck_degrade=300309` + patch 1 次 + `card.create` 仍 1 ⇒ **不另建卡**；之后每帧只走 patch、元素写入不再增长；收尾也走 patch） · 探针卡按 message_id 删净 |
