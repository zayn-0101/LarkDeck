...
- ✅ **2026-09-16 P1a 更新（实现已过门禁）**：`probe_report` 的 10 个契约键已按
  ...
  ⇒ 下表 **S1 已解决**（`session_attribution_ok` 现在会显示）；**S2、S6、S7 仍未解决**。
...
| S7 | `interrupt_session_activity` 的**核心查找名**（`gateway/run_agent_cache.py:415`）是探测盲区 | 改名 ⇒ `/stop` 后卡片**永久停在生成态**；探测仍报「无缺失」、无日志 | 子代理 |
...
> 第 2 行 S7、第 4 行 S2、第 6 行 S6、第 7 行矩阵探针**仍未做**；第 5 行 `compat.py`
> 的乐观注释也要按实际情况继续校正。
