...
| 6 | 探针 ⑮：**2.0 `button` + 组件级 `behaviors`** 真机点一次 | AGENTS 不变量 5 的「组件可以是 `button`」**只有官方文档证据，无真机点击证据**（探针只覆盖 `select_static`/`input`）；AP 自己也没测过（它在 e2e 注释里承认「V2 里的 action 会被拒 `230099`，所以这张卡从不真发」） | **M**（需用户点一次） | 这是对不变量 5 的**证据补强**，做完顺手把那句措辞改准（区分「文档已证」/「真机已证」） |
