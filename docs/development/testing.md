...
- 改过断言必须跑 `--delta`。跳过按三重指纹判断：完整锚点及其上下文、当年命中的用例名集合、
  helper / golden / 契约夹具；任一失效就重跑。只新增用例不会让旧用例名集合失效。
- `--seed-inherited <ref>` 标记代码区域与既有版本一致，不等于本轮实跑，也不能证明测试套件未变。
  发布前用 `--ledger-status` 检查缺口；`full_audit_at` 缺失或过期时补全量审计。
- `--shard i/n`（包括 `1/1`）不自行登记全量完成。分片须经 `tools/merge_ledger4.py` 核对
  当日 red-assert 名字并集覆盖全部条目、对照零假红、现场三重指纹一致，才能合并盖章。
- 合并、变基或 `git apply --3way` 后跑全量；冲突解决与普通门禁通过不能替代变异证据。
  平常未触发这些条件时，不要求每版重复全量。

### 防止假绿与误判

- 变异快照的仓库目录必须叫 `larkdeck`，且 runner 必须确认导入的就是待测树。
  `check_hooks.py` 使用加载器命名空间，不能另导入同名模块后观察另一份状态。
- 锚点存在且唯一仍不代表用例经过它。改锚点后先预检，再用 `-k` 定向复跑；必须看到
  red-assert。只命中对照的 `-k` 退出 2；`MUTATIONS` 中禁止放应归 `CONTROLS` 的对照项。
- 替换前后等价、或锚点全在注释里，要报告“变异没生效”；崩溃 / 超时不能当作有判别力。
  `test_units.py` 输出同时有 `ERROR ` 和 `FAIL` 时，分类仍为 red-crash。
- 用例先证明错误实现能进入错误分支；独立性质单独成条，避免被前面的失败遮住。
  等待须有界，用 `_await_event(event, timeout, what)` 把超时转成断言，并保证释放阻塞任务。
