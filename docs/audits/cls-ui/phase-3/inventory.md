...
| 事实面 | 状态 | 证据与动作 |
| --- | --- | --- |
| 代码 | `verified-current` | Phase 1 产品实现冻结于 `refs/audit/cls-ui/phase1e`（commit `0bf59525…`）；Phase 2 只改测试/证据，run 3 冻结于 commit `ecf11df` / tree `fd96f90f…`（当时 preflight 389/389）。v0.6.0 release 工作树新增 CLS-33，preflight 390/390（382+8）、全量 run 4 382/382。 |
