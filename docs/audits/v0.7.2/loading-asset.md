...
> ⚠️ **已被 D1′ 取代（2026-09-22）**：本文结论「**不做上传**、直接用三家共享 key」在用户
> 2026-09-22 拍板 D1′（工具行运行中也要动图、且要**仿 aiduPOP 那张**）之后**不再成立** ——
> 现在我们自制一张三点动图、用自己的 app 上传一次。本文保留为「共享 key 真机会动」这一步的
> 历史证据；现状与进度见 `docs/audits/v0.7.2/loading-asset-v2.md`。
