...
3. 页脚 = `状态 · 时长 · 模型 · ctx`，**没有 🔖**、**没有段前缀 emoji**（B1，2026-09-22）；
...
# 分片并行（**片段上限已作废**：2026-09-22 批次改为 6 片，见下方「本批更新」；无论几片都用
# **普通模式**，不要 --target-only —— 它判绿不记账，而且现在连全量章都不许盖）：
# ⚠️ 2026-09-22 历史行（2 片命令）保留作证据：
...
四种都必须清零（对照项除外）才算这一版验完。

> **本批更新（2026-09-22，v0.7.2 面板 UX）——「分片上限 2」作废**：
> 分片数**唯一诚实的提速来源是并行**（`--target-only` 已作废：它只跑声明的目标门禁、
> 别的门禁一支都不跑，抓不到「目标门禁放过、别的门禁抓住」的变异，而且**判绿不写账本**）。
> 本批用 **6 片 × 完整模式（first-red）**，每片独立 `LARKDECK_LEDGER_PATH`，实测墙钟写回
> `docs/plan-v0.7.2-panel-ux.md` §4；出现 💥 就降到 4→3 片重跑缺口并如实记录。
> 2026-09-21 那次「2 片同时跑出瞬时故障」的教训仍然有效 —— 所以：跑之前机器必须空闲、
> 每片日志独立、合并时现场重算三指纹（`merge_ledger4.py`），**不许**手工补条目。
