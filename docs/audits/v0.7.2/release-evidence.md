...
| 门禁 | 结果 | 耗时 |
| --- | --- | --- |
| `tests/test_units.py` | **277/277 passed** | ~2.8s |
...
* 账本 `tests/mutation-verdicts.json`：`471/471 可跳过（真跑过 470 + 继承 0）；待跑 0 条`。
* 每条记录同时带**代码区域指纹**（锚点 ±15 行）与**门禁文件指纹** ⇒ 不需要任何「继承」声明。
...
| **发布前终审 A/B/C** | 代码正确性 / 用户可见 / 反假绿（在 `95af186` 上重跑） | **进行中**，结论回来后补进本文件 |
