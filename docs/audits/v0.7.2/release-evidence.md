...
* 本轮实跑构成（**2026-09-22 晚，页脚模型显示名**）：**484 条全量直跑**（4 分片 121×4，
  ...
  * **同晚追加（模型名改取 models.dev 真名，`d4ad9ce`）**：用户实测纠正后，`display_model()`
    的第三档从「格式化」换成「问 Hermes 的 models.dev 解析器」。`write_golden_trace.py --check`
    = **夹具一致**（假模型 `test-model` 不在 models.dev 里）⇒ helper 指纹未变 ⇒ **不做全量**，
    改为：`--delta` 12 条 + **由 `check_hooks` 抓住的 11 条逐条重验**（改了它的页脚断言，
    用例名指纹不会自动失效）⇒ 12 条在 `d4ad9ce` 上重验 red-assert（含 `V4-62`）。
    账本 `at` 分布：472 × `f97ce19` + 12 × `d4ad9ce`；`full_audit_at` 仍 `f97ce19`（最近一次
    **全量直跑**）。
...
| 页脚：`🤖` 后面是**模型名**不是 ID | **用户实测纠正：`opencode-go` 的 `deepseek-flash` 真名是 `DeepSeek V4.1 Flash`**（我上一版格式化猜成 `DeepSeek Flash`，错的）⇒ 改为取 **models.dev 真名**（Hermes 官方解析器 + 本机缓存；provider 条目 `name == ID` 时扫注册表找厂商条目）；别名仍优先。**待用户目视新卡** | 探针卡 v2：`om_x100b64103c5fb8a8df9437166bf12fb`（`probe_footer_model.py`，含 provider + ID + 显示名对照表 + 生产 `footer_line()` 逐字输出）；v1（旧格式化口径）`om_x100b6417897c010cc4385fad759428a` |
...
3. 页脚无 🔖、无段前缀 emoji（`状态 · 时长 · 模型 · ctx`；B1，2026-09-22 起）——本文其它位置
   出现的 `⏱`/`🤖` 都是**当时**的形态记录，不是现行口径；
4. 建卡瞬间加载指示「会动、无文字」。
