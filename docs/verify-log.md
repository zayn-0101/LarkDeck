...
| **09-21 22:4x（最终账本）** | 471 | **470 实跑 + 0 继承** | 0 | 0 | 0 | 0 | 10/10 | `--ledger-status`：`471/471 可跳过（本轮真跑过 470 + 继承 0）；待跑 0 条`；每条都同时带**代码区域指纹**与**门禁文件指纹** —— 上一行的残余风险已关闭 | `tests/mutation-verdicts.json` |
