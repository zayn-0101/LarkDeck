...
2. **全量矩阵的信息价值只在「这段代码区域变过吗」** ⇒ 引入**区域指纹账本**：
   ...
   **后来被改写**的用例抓住，今天可能已经绿了。修法：每条记录**同时记门禁文件自身的指纹**
   ...
   ⚠️ **2026-09-22 再刷新**：账本现为 **482 条**（475 → 480 是 P3.1 图标批次；480 → 482 是本轮修
   `markdown.text_color`/`div.text.icon` 两个非法字段时新增 `V4-60`/`V4-61`），
   `full_audit_at=37d2ab1`、`--ledger-status` = 482/482、待跑 0；实测口径 = **4 分片并行
   ≈15 分钟 + 8 条负载 💥 定向 `-k` 复跑 ≈3 分钟**（详见上表 09-22 行与
   `~/.larkdeck-scratch/v0.7.2-fix-20260922/`）。分片起跑**必须** `start_new_session=True`
