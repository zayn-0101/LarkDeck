...
- **点选澄清卡后，主卡不再停在 `clarify · Running` / `💬 等待你的选择...`**：
  ...
  一直看起来像「点了没反应」。现在：
  1. 结构化车道识别这个边界占位，**不关主卡流**，正文改写为「⏳ 等待你的选择…」；
...
- 修复只影响结构化 CardKit 车道的 clarify 边界；旧 `patch`/降级车道行为不变。

### 门禁与证据（2026-09-24，定向）

- `test_units.py`：**338/338 passed**（新增 2 条：面板乐观更新与真实结果覆盖、
  边界保留主卡流并可在点击后刷新）。
- `run_fast.py --full`：8 步全 `[OK]`。
- `mutate_check.py --preflight`：**593/593 可用**（变异 581 + 对照 12）。
- 新增变异 `V076C-1/2` 均 `red-assert`（定向跑 2/2）。
