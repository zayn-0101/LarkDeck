...
- **元素「容量到顶」有了自己的码与处置**（R11-B2）。真机实测（新探针
  ...
  * 复用卡里已存在的元素 id ⇒ 外层同样是 `300315`，但**尾部内层码是 `300301`**
  ...
  验收：单测 **188/188** · `OVERRIDE OK` · `HOOKS OK` · `CLARIFY E2E OK` ·
...
- **滑窗写入守卫**（R11-B1）：1 秒窗口内写满 10 次逻辑写时，本帧**让出装饰批量**
  ...
  动机如实写：稳态（≈8.2 逻辑写/秒）根本触发不到它 —— 它是给 Phase C 的运行时
  `card_element.create`（会把滑窗顶到 10–14 次/秒）预留的余量闸门。
  验收：变异 `B1-1..B1-3` 全红（含「让出的写也记账 ⇒ 装饰静默冻结」「跳过当失败」
  「守卫无条件触发」）。
