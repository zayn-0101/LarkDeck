...
- **工具参数脱敏漏脱两处真形态**（审计实测）：`export KEY="值"` —— `_args_preview` 喂的是
  `json.dumps` 的产物，命令里的引号变成 `\"`，旧值类撞上第一个 `"` 就收尾 ⇒ **凭据原文完整露出**；
  `Cookie:` / `Set-Cookie:` 头 —— 四条规则一条都不覆盖它（`cookie` 原先只在 JSON **键名**规则里）。
  另修：值里含**转义引号**时只涂前半截（`{"password": "***"jwt"}`，**半截凭据同样是凭据**，
  而且看起来像处理过了）。
- **家目录规则把 URL 截断**：`https://x.com/home/dashboard` → `https://x.com~` ⇒ 用户看到一个
  **不存在的 URL**。加了左边界（只在路径起始处匹配）。
- **建实体时页脚元素不进卡**（审计实测的功能缺陷）：`_ld_footer()` 在本进程第一次 API 调用前返回
  `None`，而 `footer_text is None` ⇒ 页脚元素**不进卡**（元素表建实体时定死）⇒ **重启后第一回合
  永远没有页脚、也永远没有短码**。改成按**配置**决定要不要这个元素（要不要元素 ≠ 这一刻有没有内容）。
...
- `/larkdeck status` 三条：**已运行** / **掉回纯文本**（本回合把卡片车道让给核心的次数）/
  **错误码 top-N**（只数非零码，不同码上限 24 + `other` 桶）。
...
- 变异：`R11-1..R11-9` · `G1-1..G1-23` · `G2-1..G2-11` · `X14a/X14b/X18/X19/X20/X21/Y5/Y20`
  （共 **310** 条），逐条实测变红；其中 **12 条**是**「改坏却全绿」逼出来的**
  （验函数不验接线 / 拿被测常量算上界 / 拿同模块函数比对象 / 输入构造让错误分支不可达 /
  判据放宽 / 编号 1 基 / 扫描上限 / 条件③④ / 自检计数 / 锚点打在另一条分支上）。

### 修复（第二轮对抗审计，2026-09-15 深夜）

- **`_dur` 对脏值会编数字、甚至会抛**（真缺陷，不是断言问题）：`nan` → `int(nan)` 抛
  `ValueError`、`inf`/`-inf` → `OverflowError`；`0`/`-1` → **`'497079h14m'`**
  （"已运行 56 年"这种**看着像真数字**的脏值）；未来时间戳（`1e18`）→ `'0s'`（显示成"刚重启"）。
