...
## [Unreleased] - v0.7.2 真机反馈收敛

### 变更（用户可见）
- **页脚不再有 🔖 短码**（用户 2026-09-21 口径：「我从来没有提过这个要求」）。
  页脚字段 = 状态 · ⏱ 耗时 · 🤖 模型 · ctx 用量（对齐同类插件）。短码只保留在**日志自检行**
...
- **页脚显示「模型名」而不是模型 ID**（用户 2026-09-22 口径：「显示现在的好像是模型 ID，我想要
  做成显示模型名」）：`deepseek-v4-flash` ⇒ `DeepSeek V4 Flash`、`nvidia/moonshotai/kimi-k3` ⇒
  `Kimi K3`、`claude-sonnet-4-5-20250929` ⇒ `Claude Sonnet 4.5`（确定性格式化：已知 token 表 +
  版本号/参数量 + 尾部日期戳；不改语义、不猜名字）。别名优先：配置 `model_aliases` 精确匹配，
  其次 `~/.hermes/model_aliases.json`（`{"子串": "显示名"}`，与 `hermes-fry-cards` 同一份文件、
  子串匹配、改文件即生效）；别名文件损坏只退化成格式化，绝不弄坏页脚。
