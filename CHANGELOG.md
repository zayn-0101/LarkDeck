...
## [Unreleased] - v0.7.4 系统/命令提示静默 + 长任务面板

### 修复（登记，用户 2026-09-23 真机反馈）

- **Hermes 本地化系统/命令回复仍会带 `✅ 已完成`**：`/reset` 回复卡（`✨ 会话已重置！重新开始。…`）
  在 v0.7.3 仍被当回合卡渲染（日志 `send 判定 turn=True keys=['notify']`）。根因：命令最终回复
  与真实模型终稿同样带 `notify=True`，只能靠**内容前缀清单**识别；v0.7.4 扩展已知系统/命令前缀
  （reset/new、resume、reload-*、stop、reasoning 等）并加回归测试/变异。
- **长任务/多卡时中间卡执行详情面板空白**：原生流回退/多回合交错时 panel 快照被后续回合顶掉；
  登记 v0.7.4（复现 + 「无过程数据时沿用最后一次非空面板或不出面板」二选一）。

### 待办/探针

- `show_reasoning=true` 嵌套面板真机渲染探针（默认 false，无用户可见风险）。
- 全量重验后回填门禁数字（未跑前不写）。
