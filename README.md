...
- **面板归属靠 `turn_id` 定位**：native 流式帧带着核心给的 `turn_id`，与钩子载荷里的同源（形如 `session:task:uuid8`，跨会话唯一），所以**并发会话各查各的，不串台**。只有拿不到 `turn_id` 的回落路径（`send` / `edit_message` 传输）才退回「最近活跃会话」——这条路径下多会话并发仍可能短暂显示另一个会话的推理/工具。正文与页脚不受影响。
