...
**回退**：`git checkout <tag>` 后重启。**已发布**：`v0.7.8`（思考面板：手动展开/收起 + 轮结束自动折叠 + show_reasoning auto）、`v0.7.7`（澄清选择后主卡即时更新）、`v0.7.6`（Hermes 0.21.4 延迟平台兼容）、`v0.7.5`（长任务心跳进主卡面板 + seed 防闪旧）、`v0.7.4`（系统/命令回复静默 + 长任务面板修复）、`v0.7.3`、`v0.7.2`、`v0.7.1`、`v0.6.4` … `v0.6.0`、`v0.5.0`、`v0.4.0`、`v0.3.0`、`v0.2.0`，`v0.1.0` 是内部基线 tag（未建 Release）。
