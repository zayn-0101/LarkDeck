...
**回退**：`git checkout <tag>` 后重启。`v0.2.0` 是当前发布版；`v0.1.0` 是内部基线 tag（未建 Release）。
...
上面写的 `v0.2.0` 就是当前清单里的值。清单读不到时它会写「版本读不到」，
而不是把版本段**静默**去掉。）
...
- **`cardkit` 传输的取舍**：它换来真正的逐字打字机，代价是三条硬约束 ——
  ...
  建好、之后只改内容**（不做流式期间的结构性写入），拆开的收益是**每帧不再重发工具行**
  （推理逐字在长时，工具块不跟着一起发；实测能省 ≈2.7KB/帧），观感与「一个 markdown」一致；
