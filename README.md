...
- **`cardkit` 传输的取舍**：它换来真正的逐字打字机，代价是三条硬约束 ——
  ...
  任何一步失败（建实体 / 写元素 / 拿不到 SDK / 超硬上限）都会让**这一帧返回 `False`**，
  核心随即**停用本回合的 native 流式**、改用普通发送/编辑 —— 用户看到的仍然是**一张卡**，
  只是不再逐字（再不行才回落官方纯文本）。**消息永远不会因为卡片出错而丢。**
  ⚠️ 注意「回落」的准确含义：它**不是**「自动换成 `patch` 传输继续流式」，而是掉出 native
  流式这条路（排查「卡片怎么变成一条条消息了」的人容易在这里被带偏）。
- **cardkit 下流式期间看不到的两样东西**（结构在建实体时定死，收尾那一帧才出现）：
  ① **页脚**（`footer: true` 的上下文用量）；② 面板标题里的**模型 / 轮数 / 工具数 / 耗时**
  统计行。它们不影响正文与面板内容，但会让卡片在收尾时「跳一下」。想避免这种跳变就配
  `native_transport: patch`（那条路上页脚与统计行在流式期间就在）。
