...
| **自检命令** `/larkdeck status`（版本 / 生效传输 / 钩子 / 三条心跳记录） | ✅ 只报**有证据**的事：没记录就写「无记录」，绝不说「正常」；口径四句话（写卡数的是**帧**不是 API 次数 / 每一次真的写卡都记 / 同一帧只记一次 / 「写卡失败」只算我们发起且失败的写）与「三条记录是进程级累计」都写在卡上。⚠️ **在飞书网关里**生成中敲的命令会被排队（CLI / TUI 里可直接执行）。见下面「自检」一节 |
...
* ⚠️ **这三条记录是进程级累计**（与页脚指标同一件事）：`context._STATUS` 是模块级快照，
...
- **cardkit 下流式期间实时更新的东西与不更新的东西**（2026-09-13 更新）：
  ...
  **怎么剥**（R11-A7）：只在**能被证明**时剥 —— `帧文本 == 我们攒的正文 + "\n\n---\n" + 尾巴`
  ...
  ⚠️ 它数的是**中间帧** —— **收尾帧从设计上永不剥**（核心那一帧发的是纯累积正文，
  多剥一次就是不可逆地吞正文）⇒「N=0 而正文干净」是可能的（模型先调工具、后写回答那种回合就是）。
