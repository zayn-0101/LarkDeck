...
一条回复从第一个 token 到最终答案，始终是**同一张卡片**，原地长出来。推理过程和工具调用收进底部一个可折叠面板，正文区只放回答（核心的工具进度行在**工具执行期间**可能短暂出现，回答一开始就消失 —— 见下面 ④）。
...
| 页脚：上下文用量 + **本卡短码**（`🔖 xxxxxx`，与自检行同一串 ⇒ 截图可与日志对齐；模型/耗时已并入面板标题） | ✅ 真机渲染已确认 |
...
| **自检命令** `/larkdeck status`（版本 / 生效传输 / 钩子 / **六条记录**） | ✅ 只报**有证据**的事：没记录就写「无记录」，绝不说「正常」；口径四句话（写卡数的是**帧**不是 API 次数 / 每一次真的写卡都记 / 同一帧只记一次 / 「写卡失败」只算我们发起且失败的写）与「这些记录是进程级累计」都写在卡上。后三条是**排障用**的：`已运行`（区分「刚重启」与「跑很久了」）、`掉回纯文本`（**用户肉眼能看出**的掉卡次数，与「写卡失败」口径不同）、`错误码 top-N`（把「失败很多次」收敛成「失败在哪个码上」）。⚠️ **在飞书网关里**生成中敲的命令会被排队（CLI / TUI 里可直接执行）。见下面「自检」一节 |
...
**回退**：`git checkout <tag>` 后重启。`v0.4.0` 是当前发布版；`v0.2.0` 是首次正式发布；`v0.1.0` 是内部基线 tag（未建 Release）。
  ...
  entries:
    larkdeck:
      settings:
        ...
        progress_lines_in_body: false  # 核心的工具行不进正文（默认剥掉；工具执行期间可能短暂可见，正文只放回答、工具步骤收在面板里）
...
```
🃏 larkdeck v0.4.0 · 传输 cardkit · 钩子 7/7 已挂
...
上面写的 `v0.4.0` 就是当前清单里的值。清单读不到时它会写「版本读不到」，
而不是把版本段**静默**去掉。）
...
- **cardkit 下流式期间实时更新的东西与不更新的东西**（2026-09-13 更新）：
  ...
  那些行本来会被核心**并进流式正文**（native 流式下核心把帧文本合成成
  `正文 + "\n\n---\n" + 工具进度行`），而同一份信息面板里已经有一份**结构化**的（每个工具一行、
  带耗时与参数预览）⇒ 默认剥掉，**正文只放回答**；想恢复核心那套就把它设成 `true`。
  ⚠️ **如实说明边界**：剥的条件是「能被证明」，所以**工具执行期间那几行可能短暂出现在正文里**
  （模型先调工具、后写回答的回合尤其如此 —— 那时我们手里还没有正文可当锚，判据整体退回原样渲染），
  **回答一开始，它就是干净的**：核心下一个正文增量到达时自己会把那几行清掉。
  真机实测过一轮：一个「6 推理轮 + 6 工具调用」的回合自检行 `正文剥进度=0`，而用户回看终态「没有大问题」
  —— 那正是这条 fail-open 在按设计工作（宁可偶尔短暂可见，也绝不冒险吞掉正文）。
  **怎么剥**（R11-A7）：只在**能被证明**时剥 —— `帧文本 == 我们攒的正文 + "\n\n---\n" + 尾巴`
  ⇒ 那条尾巴只可能是核心加的（模型写下的每个字节，**包括它自己写的 `---`**，都在我们攒的正文里）。
  任何一个条件对不上（没有工具窗口 / 正文太长被冻结 / 前缀对不上 / 尾巴里没内容）就**原样渲染**：
  代价只是这一次进度行短暂可见（核心下一个正文增量到达时自己也会清掉），**绝不会吞掉正文**。
  ⚠️ 这条**不需要动核心的任何配置**（早先版本靠 `display.platforms.feishu.tool_progress: off`
  绕过，现在已撤掉）；它同时让这个一直存在但**从未真正生效**的配置键第一次真的有效
  （旧实现挂在 `format_tool_event` 上，而 Hermes 0.21.1 的生产代码**不调用**那个扩展点）。
  想核对它有没有生效：网关日志里的每回合自检有 `正文剥进度=N`（N ≥ 1 就是剥到了）。
  ⚠️ 它数的是**中间帧** —— **收尾帧从设计上永不剥**（那一帧通常是纯累积正文，多剥一次就是
  不可逆地吞正文）⇒「N=0 而正文干净」是可能的（模型先调工具、后写回答那种回合就是）。
  **一个已知例外**：某一帧发送**确定性失败**后，核心会用同一个合成文本再发一帧收尾
  （`stream_consumer_transport.py:391`）—— 那一帧带的进度行会留在卡片正文里（丑，但一个字都没丢）。
