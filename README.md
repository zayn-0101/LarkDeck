...
一条回复从第一个 token 到最终答案，始终是**同一张卡片**，原地长出来。推理过程和工具调用收进底部一个可折叠面板，正文区只放回答（核心的工具进度行在**工具执行期间**可能短暂出现，回答一开始就消失；v0.6.2 起空累积的多行进度也逐行剥离。已知例外：某一帧确定性失败后核心会用同一合成文本重发 finalize，那一帧的进度行会留下 —— 防不可逆吞正文的取舍，见下面 ④）。
...
**回退**：`git checkout <tag>` 后重启。`v0.6.3` 是当前发布版；`v0.6.2` / `v0.6.1` / `v0.6.0` 是前三版；`v0.5.0` / `v0.4.0` 是更早版本；`v0.1.0` 是内部基线 tag（未建 Release）。
...
```
🃏 larkdeck v0.6.3 · 传输 cardkit · 钩子 7/7 已挂
...
上面写的 `v0.6.3` 对应 v0.6.3 清单；清单读不到时它会写「版本读不到」，
而不是把版本段**静默**去掉。）
...
- **cardkit 下流式期间实时更新的东西与不更新的东西**（2026-09-13 更新）：
  ...
  （模型先调工具、后写回答的回合尤其如此 —— 那时我们手里还没有正文可当锚；v0.6.2 起空累积时会逐行识别核心多行进度块并剥成空/占位，识别不了仍 fail-open），
  ...
  **怎么剥**（R11-A7）：只在**能被证明**时剥 —— `帧文本 == 我们攒的正文 + "\n\n---\n" + 尾巴`
  ...
  空累积时另有更严的逐行判据（核心 verb 精确表 / 真实工具名 / 围栏 / `×N` / 光标 / `Working`；裸围栏必须与 running terminal 交叉验证；工具名单与正文累积同会话），全部行可证明才剥。任何一个条件对不上（没有工具窗口 / 正文太长被冻结 / 前缀对不上 / 尾巴里没内容）就**原样渲染**：
