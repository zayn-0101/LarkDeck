...
**回退**：`git checkout <tag>` 后重启。`v0.5.0` 是当前发布版；`v0.4.0` 是上一版；`v0.2.0` 是首次正式发布；`v0.1.0` 是内部基线 tag（未建 Release）。
...
```
🃏 larkdeck v0.5.0 · 传输 cardkit · 钩子 7/7 已挂
...
上面写的 `v0.5.0` 就是当前清单里的值。清单读不到时它会写「版本读不到」，
而不是把版本段**静默**去掉。）
