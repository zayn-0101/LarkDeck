...
| 推理文本 / 工具结果上限 + 元素溢出保护 | ✅ |
| **markdown 卫生**（只在收尾帧）：游离的 `**` 删掉、H1–H3 降级为加粗 | ✅ 模型偶发漏一个 `**`（整段被未闭合加粗包住）时，卡片正文会露出一个**字面的 `**`**；`#` 标题在卡里会变成夸张大字。现在收尾帧做两步清理：**代码区之外的游离 `**` 删掉**（不补 —— 补 `**` 会把后半段整段吞进加粗）、**H1–H3 降级为加粗**。**只作用于收尾帧**（流式帧必须严格是「累积全文的前缀链」，中途改写前缀会让用户看到文字跳变）；代码区里一个字节都不动；纯函数且**幂等** |
| **自检命令** `/larkdeck status`（版本 / 生效传输 / 钩子 / 三条心跳记录） | ✅ 只报**有证据**的事：没记录就写「无记录」，绝不说「正常」；⚠️ **仅空闲态可用**（生成中敲的命令会被当成普通输入排队）。见下面「自检」一节 |
...
**已处理 / 已过期**（卡片**不动**，弹一条 toast「这条澄清已被处理或已过期，无需重复点击」）。
⚠️ 第三态**不是「失败可重试」**（R8 审计收口时按证据改准）：网关那侧返回 `False` 的条件
**只有两条** —— entry 不存在，或它的 `Event` 已经 set（Hermes `tools/clarify_gateway.py:90-98`）
⇒ 走到这条提示时，这条澄清**已经**被处理掉或根本不存在，**再点一次永远不可能成功**
（clarify_id 是每张卡现生成的，不会被重新登记）。原来那句「请重试」是在邀请用户做一件
不可能成功的事，而且它最常见的触发者是「一次迟到的重复点击」—— 那一瞬间卡片**已经**
被前一次点击换成「已确认」，于是用户同时看到「卡片 = 已确认」+「toast = 请重试」两条
互相矛盾的信息，合理的反应是再点一次（再吃一条同样的错话），或者把答案**用文字再发一遍**
（那会变成一条新的用户消息进会话，而 agent 那边其实早就收到答案了）。
第三态为什么**不换卡**：一旦换卡，这次迟到的重复点击就会把用户的确认**退回待答** ——
而答案其实早已送达 agent（不可逆的用户可见错误）。toast 能告知失败，又不可能覆盖卡片状态。
另外三种提示各说各的话（**不塌缩成一句**）：`REJECTED_*`（答案没被接受，换个说法/选项还能救）
说「换个说法或换个选项」；`NO_PENDING` 说「已处理或已过期，无需重复提交」；空提交
（`input` 空着回车 / 多选全取消）说「没收到内容，请先选一个或输入答案」。
另一条纪律：**换卡能力要运行时探测** —— 核心那侧的 `_card_response` 若不接受卡片实参，
就自动退回「不改卡但点击仍然生效」，**并补一条 success toast**（以前这条路是完全静默的：
用户会以为没点上 → 再点一次 → 第二次必然 `not committed` → 吃一条误报），绝不抛
（点击回调跑在 SDK 线程上，抛一次会把「别人的卡」（审批卡等）的点击整条炸掉；
更糟的是异常穿透会走回**内置实现**，而内置会把这次点击的载荷做成一条合成命令
`/card button {…}` **发进会话**，卡片还不换）。
...
/larkdeck help     # 用法（含下面那条「仅空闲态」的提醒）
```

它的文本回复走我们自己的 `send()` ⇒ **自动成一张卡**：

```
🃏 larkdeck v0.2.0 · 传输 cardkit · 钩子 7/7 已挂
入站心跳：09-14 03:11:07 · 累计 42 条消息
最近写卡：09-14 03:11:09 · 累计 118 次
最近写卡失败：无记录（累计 0 次）
```

* **入站心跳**：官方钩子 `pre_gateway_dispatch`（复用已有订阅，不新增）每收到一条入站消息
  记一次。它是**与卡片无关**的证据，于是两种病可以分开：**心跳不动** ⇒ 消息根本没到插件
  （进程换了 / 平台名被别的插件抢走）；**心跳在动而「写卡」不动** ⇒ 消息到了、卡片却没发出去
  （配置关了 / 帧全失败）。
* **写卡次数**：只统计**真的写出去**的动作 —— 建卡（首发 / 建实体）、正文元素写、降级后的整卡
  替换、收尾替换。**节流跳过**的那一帧与**文本没变**的去重帧**不算**（它们一个字节都没写）。
* **写卡失败**：帧失败的**原始原因**（截断到 120 字符、换行折叠）。日志里那条 WARNING 有 30 秒
  限流，而用户看不到日志 —— 「刚才那回合为什么掉成纯文本」只有这张卡答得出来。
* ⚠️ **没记录就写「无记录」，绝不写「正常」**：一张永远说健康的自检卡与一张坏掉的自检卡，
  在用户眼里长得一模一样（`docs/lessons.md` 推论 6）。
* ⚠️ **仅空闲态可用**：命令派发只挂在核心的 **idle 路径**上
  （`gateway/run_inbound.py` 的 `_hm_dispatch_idle_commands`），生成回答时敲的命令会被当成
  **普通输入**排队 —— 不是被忽略，但也不会立刻回卡。
* 注册不到不影响卡片功能：老版本 Hermes 没有 `ctx.register_command()` 时，启动自检会**如实
  写着** `· /larkdeck 命令未注册（原因）`，而不是让你敲了没反应。
...
- **cardkit 下流式期间实时更新的东西与不更新的东西**（2026-09-13 更新）：
  ...
  那些行本来会被核心**并进流式正文**，而同一份信息面板里已经有一份**结构化**的（每个工具一行、
...
- **每帧的**元素写**预算是 2 次**（装饰一次 `card.batch_update` + 正文一次
  ...
  重发（0.1/0.3/0.6，最多 4 次尝试，`uuid` 不变所以幂等）⇒ 单帧最坏 **12 次调用、约 3 秒**。
