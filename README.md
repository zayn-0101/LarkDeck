...
| 过程面板（CLS 观感）：`💭 思考 1.6s · 🛠️ 工具执行 · 3 步`；工具行 = **emoji 内联**（📄/🛠️/🔍…，v0.7.2 起：`div.icon` 会被客户端顶部对齐、用户真机选版换成内联）+ 加粗**工具名**（`read_file` / `terminal` / `web_search` …）+ 耗时 + 绿色 `Succeeded` / 青绿 `Running` / 红色 `Failed`·`Blocked`·`Timed out` / 灰色 `Cancelled`·`Skipped`，命令或 skill 名另起一行灰色小字 | ✅ 数据来自官方钩子；工具行动作词/状态词为**语言固定边界**（markdown 不承载 `i18n_content`）。✅ v0.6.2 默认 `panel_color_tags: true`（官方 Card 2.0 markdown 文档确认 `<font color>` 与色板）+ `text_profile: compact`（面板/页脚 12px notation、正文 normal）。⚠️ 真机视觉仍需用户截图确认；客户端不认时可显式设回 false 无色降级 |
