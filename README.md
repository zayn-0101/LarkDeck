...
**回退**：`git checkout <tag>` 后重启。`v0.3.0` 是当前发布版；`v0.2.0` 是首次正式发布；`v0.1.0` 是内部基线 tag（未建 Release）。
...
```
🃏 larkdeck v0.3.0 · 传输 cardkit · 钩子 7/7 已挂
...
上面写的 `v0.3.0` 就是当前清单里的值。清单读不到时它会写「版本读不到」，
而不是把版本段**静默**去掉。）
...
- **每帧的**元素写**预算是 2 次**（装饰一次 `card.batch_update` + 正文一次
  ...
  它是给「将来元素会动态增长」预留的余量闸门，**稳态下不会触发**（≈8.2 < 10）；
