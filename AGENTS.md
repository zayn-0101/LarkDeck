...
⚠️ **注释里的「理由」也要能站得住**（2026-09-16 第四轮审计实测）：`_strip_core_progress`
    ...
    ⚠️ **残留（如实登记，2026-09-16）**：独立成条的是**我们自己**的性质（剥出来的必是帧前缀），
    那个「同源」**外部前提至今没有判据**（只有一段引用核心行号的论证）⇒ 要真钉住它得
    **驱动核心的投递路径**（假客户端跑一轮真流式，比对「钩子收到的 text」与核心 `_accumulated`），
    **未做**，登记在 `docs/plan-v1.md` 附录 F 末尾（= `docs/lessons.md` 推论 37 的残留）。
