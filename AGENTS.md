...
- **markdown 卫生只作用在「完整文本」的写入上**（`cards.sanitize_markdown`，R6a；
  R6a 审计中-3 把口径从「只在收尾帧」改准 —— 原来只有 native 收尾那一帧做，
  于是同一段模型输出在**正常收尾**与**被 `/stop`** 两条路上长得不一样）。核心的契约是
  「流式帧的 `text` 是**累积全文**」⇒ 任何一帧改写前缀，都会让用户看到文字**跳变**
  （而且核心自己只在收尾那一帧补未闭合的围栏）。**判据是「这份文本是不是完整文本」**，
  不是「这是哪条调用路径」：
  * **做**：`send()`（整条消息）、`edit_message(finalize=True)`（收尾整卡）、
    `/stop` 的中止重绘（正文来自 `_ld_streams` / `_ld_state` 的追踪表 ⇒ 卫生后与收尾帧一致）、
    native 收尾那一帧（`cardkit` 与 `patch` 两条传输都走它）；
  * **不做**：`edit_message(finalize=False)` 与 CardKit 的**元素帧** —— 它们写的是累积帧的
    中间态，改一个字节就断前缀链（单测里两处都有**反向**断言）。
  四条纪律：① **代码区内容一个字节不许动**（围栏 / 行内代码里的 `#`、`**` 都是正文）——
  围栏识别包含**未闭合围栏**（延伸到文末：核心的「边界收尾」会把没闭合的累积文本直接送出来，
  `gateway/stream_consumer.py:484/486`）、**四反引号及以上**（CommonMark 的嵌套写法）、
  **`~~~` 围栏**；② 游离的 `**` **删掉而不是补齐**（补一个会把后半段吞进加粗），而且删的是
  **第一个**候选（删最后一个会把**后文合法加粗对的闭合标记**拆掉、把中间整段吞进加粗 ——
  审计实测：那比不改更糟）；③ **顺序不能反**（先删游离 `**`、再降级标题）；
  ④ **幂等**（`f(f(x)) == f(x)`）—— 这条**曾经是假的**（最小复现
  `'## 用 ``` 开围栏\n## 建议\n```\ncode\n```\n'`），现在**标题体里有反引号/`~~~` 时不做
  加粗降级**（去掉行首标记、不插任何字符）⇒ 审计的两条语料上幂等成立：块拼接穷举
  2379 条 **0 条**、现实语料 6 万篇 **0 条**、随机 20 万条 **0 条**（修之前是 4 / 19.29% / 369）。
  ⚠️ 它**不是**全域不变量：畸形输入（例如「一个 `**` 夹在两对**空**行内代码之间」）会让
  `_code_spans` 的**跨度划分**变化（用户看到的字一个都没变）。文档口径因此是
  「**在真实语料上成立**」，不是「对所有输入成立」。
...
- **`/larkdeck status` 是自检的唯一入口**（`ctx.register_command`，公开 API）：报版本
  ...
  ③ **口径是「帧」不是「次」**（R9 审计中-5）：一帧 = +1，**哪怕这一帧做了多次写** ——
     cardkit 一帧最多 3 次逻辑写（装饰 batch + 正文 content + 限频预览）、seed 帧是 2 次网络调用
     （建实体 + 发实体卡）、一次逻辑写撞限流最多重发 4 次 HTTP。所以卡片与文档一律说
     「**帧真的有写出**」，绝不说「写了 N 次 API」；
  ④ **记账挂在低层写卡收口点**（`_ld_send_card` / `_ld_update_card` 成功处）+
     **两条帧路径特例**（cardkit 的 seed 建实体、每帧元素写 —— 它们不经那两个原语）。
     这是 R9 审计中-1 的修法：以前 7 个调用点全在 `_ld_stream_frame` 里，于是
     `send()` 首发 / 非流式 `edit_message` / `send_clarify` / `/stop` 重绘**真的写了卡却一个数都不加**
     （实测 `send()` 成功 ⇒ 三行全「无记录」，而用户 DM 里明明有卡）。反过来的纪律同样硬：
     **同一帧不许记两次**（DEGRADE 那一帧既写元素又整卡 patch、seed 是两次网络调用，都只 +1）——
     由「记账下沉到原语」+「帧路径特例单独收口」保证，不靠调用方记得别多写一句。
  ⚠️ **「写卡失败」的口径**（R9 审计中-2）：它只统计**我们真的发起过一次写、而它失败了**的帧。
  它**不等于**「核心收到过几个 `False`」—— `send_stream_frame` 返回 False 还有一支是
  「没有活跃流可收尾 ⇒ 按官方契约把控制权交还核心，核心回落 edit/send」（一个写请求都没发，
  消息照常发出去）。那一支是**正常路径**，进失败账本会让健康回合在排障卡上显示一条指向
  不存在的写卡动作的原因。
  ⚠️ **这三条记录是进程级全局、跨会话共享**（R9 审计中-4，与页脚指标同一件事）：
  `context._STATUS` 是模块级 dict，多会话并发时卡片上的数字含**别的会话**的那部分
  （「最近写卡失败：1 次 · <原因>」可能是别人的失败）。所以卡片上必须带一句口径说明
  （i18n `cmd.scope`）；排障时别拿别人会话的失败原因查自己的卡。要真做对得从钩子载荷的
  `session_id` 分桶（与页脚同一件事，**未做**）。
  ⚠️ 命令派发只挂在核心的 **idle 路径**上 ⇒ **在飞书网关里，生成回答期间敲的命令会被排队**
  （当成普通输入，回合结束才派发）；**别写成绝对规则**（R9 审计低-1）：CLI / TUI 里
  （`cli.py::_run_plugin_slash_command`、`tui_gateway/methods_tools.py`）**直接调处理器**、
  立刻回卡。注册不到不影响卡片功能，但启动自检必须**如实**写「命令未注册（原因）」，
  而那句自报由 `check_override.py` 用**核心自己的 getter**（`get_plugin_command_handler`）核对
  （R9 审计中-6：它以前可以硬编码成永远成立而四门禁全绿）。
