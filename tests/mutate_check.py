...
MUTATIONS = [
    ...
    ("V076C-2-澄清点击后不乐观标 ok（面板一直 Running）", "core/panel.py",
     ...
     "test_units"),
    ("V079-1-拿不到显示名时仍拼破折号署名（已确认卡露出空的「— 」尾巴）", "core/cards.py",
     '    return f"{line}\\u3000\\u2014\\u3000{user_name}" if user_name else line',
     '    return f"{line}\\u3000\\u2014\\u3000{user_name}"',
     "test_units"),
    ("V079-2-点击者名字退回 open_id（已确认卡显示一串 ou_ 字符）", "core/adapter.py",
     '            return str(self._get_cached_sender_name(open_id) or "")',
     '            return str(self._get_cached_sender_name(open_id) or open_id)',
     "test_units"),
